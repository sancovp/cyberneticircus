...
* **The Code Geometry**:
  * In [engine.py](file:///Users/isaacwr/claude_code/cyberneticircus/cyberneticircus/engine.py), the `tick_cybernet_turn` function pulls the active `StateMachine` associated with a `:Cybernet`.
...
* **The Code Geometry**:
  * The `/api/graph` endpoint in [web_server.py](file:///Users/isaacwr/claude_code/cyberneticircus/cyberneticircus/web_server.py) implements a localized BFS traversal starting exclusively at the selected Cybernet's node ID.
