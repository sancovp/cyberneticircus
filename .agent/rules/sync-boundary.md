...
1. **Host-First Edits**: All editing and coding must happen in the host workspace `/Users/isaacwr/.gemini/antigravity/scratch/`. Never edit files directly inside the container.
...
3. **Prompt Promotion**: Once a task is complete and tests pass, immediately sync the changed files back to the container:
   `docker cp /Users/isaacwr/.gemini/antigravity/scratch/{file} antigravity_python_dev:/workspace/{file}`
