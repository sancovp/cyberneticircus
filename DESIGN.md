...
### A. Layer 2: Meta-Compile (refined)
...
### C. Becoming Jani (the entry point for any agent)
...
### D. Minimax in the Frontend (the animation layer)

in the frontend (the visualizer + interactive book per §10.B), **any domain can be animated by a Cybernet**. the controls: turn a domain (a mind palace, a page, a state machine, a node) into a Cybernet you can talk to standalone.
...
### E. The Current Best Architecture (synthesis)
...
| **Frontend** | any domain is animatable as a standalone Cybernet. minimax is the runtime; the visualizer is the control surface. | the user, minimax | **not started** (blocked on §11.6 [ ] visualizer migration) |

### F. Open Questions (to cohere next session)
