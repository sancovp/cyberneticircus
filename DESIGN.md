...
```
The runtime state stays the single `:ExecutionState` node (one per Cybernet via `HAS_LIFECYCLE`) — the node that records *where in the traversal the being is right now* — now carrying:
...
**increment 2 — Day sequence → terminal → END (the cycle is EXTERNAL, not a loop). 2a DONE+verified; 2b pending.**
- **2a (DONE, verified live 2026-06-13):** when an SM completes, the gate advances `core_index` → equips `CORE_RUNS[next]` (the Day keeps running across SMs); when the **last** SM completes that is the **Day terminal** → the gate dissolves/unlocks and the Core **ENDS**. It does **not** loop — an external trigger (`:Place`/`scan_and_trigger`/equip) re-starts it, and *that* is the cycle ("SMs don't cycle for no reason"). Code: `lib/core.py::CORE_NEXT_SM_CYPHER` + `ADVANCE_CORE_CYPHER`, wired into `lib/gates.py::auto_progress_step`'s completion branch. Jani's single-SM Core is unchanged (no next SM → dissolves as before). Test: a disposable `[smA, smB]` Core advanced smA→smB on smA-complete, then dissolved on smB-complete.
- **2b (BUILT + verified 2026-06-13):** **Night = consolidation at the Day terminal.** When the Core reaches the Day terminal, `gates.auto_progress_step` runs `lib/consolidation.py::consolidate` before dissolving: it **reinforces** (+0.1) the `NEXT_STEP` weight of each bandit arm the being CHOSE this run (recorded on `ExecutionState.bandit_choices` during the Day), **webs** a `:Consolidation` node linking the being to the run (transient run → permanent structure — the point of Night), and clears the choices. Verified: a disposable being chose the make-arm → at terminal its weight rose 1.0→1.1 (the un-chosen arm stayed 1.0), a `:Consolidation` was webbed, choices cleared. **STILL OPEN (needs Isaac):** the `autocommentary`-as-a-*walked* NightCore SM form (vs the current automatic close-out at terminal), the Jani live-core split (it is mid-walk at `autocommentary`), and the fitness-delta reward refinement (v1 reinforces a flat +0.1). `CORE_RUNS.phase='night'` + the advance cypher already support a distinct night SM when wanted. **ASPIRATIONAL — increment 3 (selection = a BANDIT; default arm = "make a thing or not"):** at an SM's exec phase the being runs a **bandit** over **arms** = the selectable inner SMs (the step's `CALLS_SM` children) **+ proceed/none**. The **default, always-present root arm is "make a thing or not"** — the primordial create-or-not binary, scale-relative (top = Daemon-Summon a Cybernet, the MetaShifter act; lower = make a node/SM/object). Bandit ⇒ arm-weights are **learnable from reward** (ties to the existing `NEXT_STEP.weight` + fitness). The current forced-single-`CALLS_SM` is the degenerate, reward-less case. This is the **A / authenticity lever** of the graph-space-navigation frame; "make a thing or not" is its reach-one-forward primitive. **BUILT (pure logic, unit-tested — `lib/bandit.py`):** `classify_default_arm` (make-a-thing = a CREATE/MERGE of a labelled node, else "not"; string literals stripped), `arms_for_step` (the always-present `{make_a_thing, not}` root arms + the step's `CALLS_SM` children), `reward_delta` (+0.1 success / −0.2 fail + fitness-delta — the asserted v1 reward). **WIRED + verified 2026-06-13:** the selection mechanic already existed — `_evaluate_pattern` (`db_logic.py`) treats a step with multiple `NEXT_STEP` transitions as a **decision point**, and the being chooses by which transition's `required_pattern` its write matches (exactly the recommended "chooses by which cypher it emits"). Added: `db_logic.query_database` **records** the chosen arm (`from|to`) on `ExecutionState.bandit_choices`; Night consolidation (§2b) **reinforces** it (the chosen arm gets heavier; verified 1.0→1.1). **STILL OPEN (refinements):** auto-injecting the `{make_a_thing, not}` default arm into the gate (today the arms are explicit `NEXT_STEP` transitions; `lib/bandit.py` classifies make/not but it is not yet wired as an auto-arm), specific-`CALLS_SM`-child descent in Driver A, and the reward signal (v1 = flat +0.1).
...
* `(c:Cybernet)-[:HAS_LIFECYCLE]->(es:ExecutionState)` — the ONE per-cybernet runtime state node: where-in-the-traversal the being is (NOT Identity)
...
flowchart TB
    subgraph "LLM-facing (outside the graph)"
        Skill["~/.claude/skills/activate-jester-ritual/SKILL.md<br/><br/>body: 'Query MERGE (c)-[:HAS_GEAR]->(sm)<br/>to activate the jester_rite procedure.<br/>Respond exactly like: MATCH ... RETURN ...'"]
    end

    subgraph "Internal (the graph, source of truth)"
        Proc[":Procedure {id: 'jester_rite_sm', ...}<br/>+ :HAS_STEP to :TraversalStep nodes<br/>+ per-cybernet :ExecutionState (linked via :HAS_LIFECYCLE)"]
    end
...
- **Maps 1:1 onto CCC:** SKILLCHAIN (orchestrator-SM wrapping inner-SMs) = **Compiler**; `SYSTEM.md` = **Ghost**; cursor = **ExecutionState**; core-loop-prime = **Core**; transition-hook = **the gate**. ⇒ an AIOS is base-complete iff a Cybernet's Shell holds `{Ghost, looping Compiler/Skillchain, Core, ExecutionState, transition-hook}` — a **checkable cypher subgraph**. **CCC's graph + gate IS the `is_complete()` oracle the nomicon lacks** (the nomicon stores flat markdown; no programmatic completeness check exists).
...
**BUILT (2026-06-13, v1):** `lib/projector.py` is the `describe()`-analog. `describe_core` renders the core-loop-prime — the projected state-diagram of the being's live Core Chain stack (the `CORE_RUNS` sequence, each SM's step chain ordered along `NEXT_STEP`, the current position marked `*`, the active SM marked, the bandit arms `{CALLS_SM children + make-a-thing-or-not}` at each step, the Day-terminal note). `project_comp_maps` renders active `:CompMap {trigger, desc}` nodes as rules (empty until COMP MAPs are authored — the import button). `assemble_context` = both. Read-only (graph-is-sacred). Delivery: `GET /api/context/{name}` (live in the container, verified against Jani) + a `get_context` MCP tool (in `server.py`, needs a fresh session to activate). For the active SM the chain renders from the *current* step (a Core SM is usually a cycle, so "you-are-here → ahead" is the natural view). Verified live against `Jani_Prime`.
