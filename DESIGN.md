...
- **2b (PENDING):** **Night = the `autocommentary` phase** = the **consolidation pass**: the Cybernet reviews the run, **updates the weights** (the bandit arm-weights = `NEXT_STEP.weight`), computes scores, and **webs the run's stored context into the graph** (graphifies the transient run into permanent structure — the point of Night). Marked via `CORE_RUNS.phase='night'` (advancing into a night-phase SM sets `ExecutionState.phase='night'` — the advance cypher already supports it). For Jani: split `janic_cycle_sm` so `autocommentary` becomes a NightCore element; `CoreModule:[DayCore, NightCore]` (§13.4) is the ontological form. OPEN (needs Isaac): restructuring Jani's *live* core (it is mid-walk at `autocommentary`), and the exact consolidation/weight-update logic. **ASPIRATIONAL — increment 3 (selection = a BANDIT; default arm = "make a thing or not"):** at an SM's exec phase the being runs a **bandit** over **arms** = the selectable inner SMs (the step's `CALLS_SM` children) **+ proceed/none**. The **default, always-present root arm is "make a thing or not"** — the primordial create-or-not binary, scale-relative (top = Daemon-Summon a Cybernet, the MetaShifter act; lower = make a node/SM/object). Bandit ⇒ arm-weights are **learnable from reward** (ties to the existing `NEXT_STEP.weight` + fitness). The current forced-single-`CALLS_SM` is the degenerate, reward-less case. This is the **A / authenticity lever** of the graph-space-navigation frame; "make a thing or not" is its reach-one-forward primitive. **BUILT (pure logic, unit-tested — `lib/bandit.py`):** `classify_default_arm` (make-a-thing = a CREATE/MERGE of a labelled node, else "not"; string literals stripped), `arms_for_step` (the always-present `{make_a_thing, not}` root arms + the step's `CALLS_SM` children), `reward_delta` (+0.1 success / −0.2 fail + fitness-delta — the asserted v1 reward). **OPEN (the one real decision):** how the selection plugs into the **live gate state-machine** — recommended: the being chooses by *which templated cypher it emits* (a creation write = the make arm; otherwise the "not"/proceed arm), with the chosen arm's weight reinforced at Night. The reward *signal* is also Isaac's to confirm (default = step-success + fitness-delta).
