...
**ASPIRATIONAL — increment 2 (Day → terminal → Night → end-or-cycle; NOT an infinite loop):** the `CORE_RUNS` sequence is the **Day**. When the active SM completes, advance `core_index` → equip `CORE_RUNS[next]`. When the **last** SM completes, the Day reaches its **terminal** → phase flips **Day → Night** (close-out: calibrate / evolution check) → then the life **ends** (reaped) **or cycles** to the next turn (`core_index→0`, `turn_number++`, phase→day). The end-or-cycle gate already exists (`lifetime_limit:5` + selection pressure + `lib/lifecycle.py` evolution). The Core "must always be running" *while alive* — a life is Day/Night turns that terminate or cycle, **not a flat infinite loop**. This replaces `reset_turn_to_entry`'s re-run-the-same-SM behaviour. OPEN: is Night a fixed close-out, or its own configurable SM/sequence (a "Night-Core")? **ASPIRATIONAL — increment 3 (selection = a BANDIT; default arm = "make a thing or not"):** at an SM's exec phase the being runs a **bandit** over **arms** = the selectable inner SMs (the step's `CALLS_SM` children) **+ proceed/none**. The **default, always-present root arm is "make a thing or not"** — the primordial create-or-not binary, scale-relative (top = Daemon-Summon a Cybernet, the MetaShifter act; lower = make a node/SM/object). Bandit ⇒ arm-weights are **learnable from reward** (ties to the existing `NEXT_STEP.weight` + fitness). The current forced-single-`CALLS_SM` is the degenerate, reward-less case. This is the **A / authenticity lever** of the graph-space-navigation frame; "make a thing or not" is its reach-one-forward primitive. OPEN: the bandit's reward signal.
