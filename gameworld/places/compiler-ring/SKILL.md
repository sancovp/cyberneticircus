...
- **Pass a gate** — issue Cypher through the `query_database` MCP tool (the sanctioned transport; `execute.sh`/raw `curl` are legacy raw HTTP) with `cybernet_name` set; if your query matches the active step's `required_pattern`, the step auto-progresses and the result carries a `_state_machine_event`. This is action validation: the graph judges your spell. (Fixed 2026-06-13, DESIGN §11.8: the gate and auto-progress now bind to the live `HAS_LIFECYCLE → ExecutionState` chain that `equip` creates — a non-matching write returns **403** carrying the required regex, a matching write passes and auto-advances `CURRENT_STEP`; no manual GM stepping. The old `HAS_TRAVERSAL` caveat is obsolete.)
